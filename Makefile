# This Makefile is for the JSON::Streaming::Reader extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/JSON/Streaming/Reader.pm]
#     AUTHOR => q[Martin Atkins <mart@degeneration.co.uk>]
#     NAME => q[JSON::Streaming::Reader]
#     PREREQ_PM => { IO::Scalar=>q[0], IO::File=>q[0], B=>q[0], Carp=>q[0] }
#     VERSION_FROM => q[lib/JSON/Streaming/Reader.pm]
...
NAME_SYM = JSON_Streaming_Reader
VERSION = 0.01
...
FULLEXT = JSON/Streaming/Reader
BASEEXT = Reader
PARENT_NAME = JSON::Streaming
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/JSON/Streaming/Reader.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/JSON/Streaming
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/JSON/Streaming
...
TO_INST_PM = lib/JSON/Streaming/Reader.pm \
	testthing.pl

PM_TO_BLIB = lib/JSON/Streaming/Reader.pm \
	blib/lib/JSON/Streaming/Reader.pm \
	testthing.pl \
	$(INST_LIB)/JSON/Streaming/testthing.pl
...
DISTNAME = JSON-Streaming-Reader
DISTVNAME = JSON-Streaming-Reader-0.01
...
manifypods : pure_all  \
	lib/JSON/Streaming/Reader.pm \
	lib/JSON/Streaming/Reader.pm
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) \
	  lib/JSON/Streaming/Reader.pm $(INST_MAN3DIR)/JSON::Streaming::Reader.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         JSON-Streaming-Reader' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.01' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: lib/JSON/Streaming/Reader.pm' >> META_new.yml
	$(NOECHO) $(ECHO) 'installdirs:  site' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '    B:                             0' >> META_new.yml
...
ppd:
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Read JSON strings in a streaming manner</ABSTRACT>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  lib/JSON/Streaming/Reader.pm blib/lib/JSON/Streaming/Reader.pm \
	  testthing.pl $(INST_LIB)/JSON/Streaming/testthing.pl 
